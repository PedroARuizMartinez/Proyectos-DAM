...
public class GestorProducte {
    ...
    public List<Producte> obtenirProductes() {
        ...
        List<Producte> productes = query.getResultList();
        return productes;
    }
     /**
    ...
    public void eliminar(int id) throws GestorException {
        //TODO completar el metode
        if(em.find(Producte.class, id) == null) {
            throw new GestorException("No existe el codigo");
        } else {
            em.getTransaction().begin();
            em.remove(em.find(Producte.class, id));
    ...
    public void inserir(Producte prod) throws GestorException {
        //TODO completar el metode
        if(em.find(Producte.class, prod.getIdProd()) != null) {
            throw new GestorException("Producto duplicado");
        } else {
            em.getTransaction().begin();
            em.persist(prod);
            em.getTransaction().commit();
    ...
    */
    public void modificar(Producte prd) throws GestorException{
        //TODO completar el metode
        if(em.find(Producte.class, prd.getIdProd()) == null) {
            throw new GestorException("No existe el Producto");
        } else {
            em.getTransaction().begin();
            em.merge(prd);
            em.getTransaction().commit();
